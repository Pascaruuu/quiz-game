...
function getCategory(meaning: string): string {
  ...
  if (/work|business|office|meeting|project|task|employee|staff|manager|client|customer/.test(m)) return 'work';
  if (/computer|software|system|data|file|version|app|network|server|database/.test(m)) return 'tech';
